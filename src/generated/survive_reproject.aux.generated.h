...
static inline void gen_quat2axisangle(FLT *out, const FLT *q) {
	...
	const GEN_FLT obj_qj = q[2];
	const GEN_FLT obj_qk = q[3];
	const GEN_FLT x0 = 1e-10 + (obj_qw * obj_qw) + (obj_qi * obj_qi) + (obj_qj * obj_qj);
...
static inline void gen_quat2axisangle_jac_q(FLT *out, const FLT *q) {
	...
	const GEN_FLT x2 = obj_qi * obj_qi;
	const GEN_FLT x3 = 1e-10 + x2 + x0 + x1;
	const GEN_FLT x4 = atan2(x3, obj_qw);
	const GEN_FLT x5 = x3 * x3;
	const GEN_FLT x6 = x4 * (1. / x5);
	const GEN_FLT x7 = 4 * x6;
	const GEN_FLT x8 = x7 * obj_qw;
	const GEN_FLT x9 = 1. / (x0 + x5);
	const GEN_FLT x10 = 1. / x3;
	const GEN_FLT x11 = 2 * x10;
	const GEN_FLT x12 = x0 * x9 * x11 * (2 + (-1 * (1. / x0) * x3));
	const GEN_FLT x13 = x4 * x11;
	const GEN_FLT x14 = x9 * x10 * obj_qw;
	const GEN_FLT x15 = 4 * x14;
	const GEN_FLT x16 = obj_qj * obj_qi;
	const GEN_FLT x17 = (x15 * x16) + (-1 * x7 * x16);
	const GEN_FLT x18 = 4 * x1;
	const GEN_FLT x19 = x7 * obj_qk;
	const GEN_FLT x20 = x15 * obj_qk;
	out[0] = (x12 * obj_qi) + (-1 * x8 * obj_qi);
	out[1] = (x2 * x15) + (-1 * x2 * x7) + x13;
	out[2] = x17;
	out[3] = 0;
	out[4] = (x12 * obj_qj) + (-1 * x8 * obj_qj);
	out[5] = x17;
	out[6] = x13 + (x14 * x18) + (-1 * x6 * x18);
	out[7] = 0;
	out[8] = (x12 * obj_qk) + (-1 * x19 * obj_qw);
	out[9] = (x20 * obj_qi) + (-1 * x19 * obj_qi);
	out[10] = (x20 * obj_qj) + (-1 * x19 * obj_qj);
	out[11] = x13;
}
